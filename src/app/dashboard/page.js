...
// ── Stat Card ─────────────────────────────────
function StatCard({ icon, label, value, sub, subColor, accent = 'green', loading }) {
  const accents = {
    green: { bar: `linear-gradient(90deg, ${C.green}, ${C.greenLight})`, val: C.greenGlow },
    gold:  { bar: `linear-gradient(90deg, ${C.gold}, ${C.goldLight})`,   val: C.goldLight },
    red:   { bar: `linear-gradient(90deg, ${C.red}, ${C.redLight})`,     val: '#E88080'   },
    blue:  { bar: `linear-gradient(90deg, ${C.blue}, ${C.blueLight})`,   val: C.blueLight },
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: C.textMuted, marginBottom: 6 }}>{label}</div>
      {loading
        ? <Skeleton h={32} w="60%" radius={6} />
        : <div style={{ fontSize: 30, fontWeight: 800, color: a.val, lineHeight: 1, marginBottom: 4 }}>{value}</div>
      }
      {sub && !loading && (
        <div style={{ fontSize: 12, color: subColor || C.textMuted, marginTop: 4 }}>{sub}</div>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
      {alerts.map((a, i) => {
        ...
        const bg      = isRed ? C.redFaint   : C.amberFaint;
        const border  = isRed ? '#7B1F1F'    : '#7A4A10';
        const color   = isRed ? '#F0A0A0'    : C.amberLight;
        const icon    = isRed ? '🚨'         : '💉';
        return (
          <div key={i} style={{
            ...
            {a.batchId && (
              <Link href={`/dashboard/batches/${a.batchId}`} style={{
                fontSize: 11, fontWeight: 600, color, textDecoration: 'underline',
              }}>View →</Link>
            )}
          </div>
        );
    ...
    <tr>
      <td style={{ padding: '14px 20px' }}>
        <Link href={`/dashboard/batches/${batch._id}`} style={{ textDecoration: 'none' }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: C.textPrimary, marginBottom: 2 }}>{batch.name}</div>
          <div style={{ fontSize: 11, color: C.textMuted }}>Started {new Date(batch.startDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
        </Link>
      </td>
      <td style={{ padding: '14px 20px' }}>
      ...
      <td style={{ padding: '14px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: mortality > 3 ? '#E88080' : C.greenGlow }}>
            {mortality}%
          </span>
        ...
        <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>{fmtNum(batch.currentAlive)} alive</div>
      </td>
      <td style={{ padding: '14px 20px' }}>
        <Badge color={statusColor}>
          {batch.status === 'active' && mortality > 3 ? '● Critical' : '● ' + (batch.status?.charAt(0).toUpperCase() + batch.status?.slice(1))}
        </Badge>
      </td>
      <td style={{ padding: '14px 20px' }}>
        <Link href={`/dashboard/batches/${batch._id}`} style={{
          ...
          border: `1px solid ${C.forestBorder}`,
          borderRadius: 6, background: C.forestSurface2,
        }}>View →</Link>
      </td>
    </tr>
  );
...
// ── Vaccine Row ───────────────────────────────
function VaccineRow({ v }) {
  const isOverdue  = !v.isDone && new Date(v.scheduledDate) < new Date();
  const daysUntil  = v.daysUntil;
  const color      = isOverdue ? 'red' : daysUntil <= 1 ? 'amber' : 'green';

  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 12,
      padding: '12px 0', borderBottom: `1px solid ${C.forestBorder}`,
    }}>
      <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: C.textPrimary, marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {v.vaccineName}
        </div>
        <div style={{ fontSize: 11, color: C.textMuted }}>{v.batchName}</div>
      </div>
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        ...
        <div style={{ fontSize: 10, color: C.textMuted, marginTop: 3 }}>
          {new Date(v.scheduledDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ color: p.color, marginBottom: 2 }}>
          {p.name}: {p.name?.includes('Revenue') || p.name?.includes('Expenses') || p.name?.includes('Profit')
            ? fmt(p.value) : p.value}
        </div>
      ))}
    </div>
...
export default function DashboardPage() {
  ...
  const [batchChart,   setBatchChart]   = useState(null);
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          <h1 style={{
            ...
            fontSize: 28, fontWeight: 700, color: C.textPrimary, marginBottom: 4,
          }}>
            {timeOfDay()}, {firstName} 👋
          </h1>
          <p style={{ fontSize: 13, color: C.textMuted }}>
            {new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
            {d?.summary?.activeBatches > 0 && ` · ${d.summary.activeBatches} active batch${d.summary.activeBatches > 1 ? 'es' : ''}`}
          </p>
        </div>
      ...
      {/* ── Alerts ── */}
      {d?.alerts?.length > 0 && <AlertStrip alerts={d.alerts} />}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
        <StatCard
          ...
          sub={loading ? '' : `${fmtNum(d?.summary?.totalBatches)} total batches`}
        ...
        />
        <StatCard
          icon="💰" label="This Month Revenue"
          value={loading ? '' : fmt(d?.finance?.thisMonth?.revenue)}
          sub={loading ? '' : `${d?.finance?.thisMonth?.isProfit ? '↑' : '↓'} ${fmt(Math.abs(d?.finance?.thisMonth?.profit))} ${d?.finance?.thisMonth?.isProfit ? 'profit' : 'loss'}`}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, marginBottom: 24 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            padding: '16px 22px', borderBottom: `1px solid ${C.forestBorder}`,
          }}>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: C.textPrimary }}>Revenue vs Expenses</div>
              <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>Last 6 months</div>
            </div>
            <div style={{ display: 'flex', gap: 16 }}>
              {[
                { key: 'revenue',  color: C.greenGlow,  label: 'Revenue'  },
                { key: 'expenses', color: '#E88080',     label: 'Expenses' },
                { key: 'profit',   color: C.goldLight,   label: 'Profit'   },
              ].map(l => (
                <div key={l.key} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: C.textMuted }}>
          ...
          <div style={{ padding: '16px 8px 8px' }}>
            {loading ? (
              ...
              <div style={{ height: 220, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                ...
                <p style={{ fontSize: 13, color: C.textMuted }}>No financial data yet.</p>
                <p style={{ fontSize: 12, color: C.forestMuted }}>Log expenses and sales to see your chart.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            padding: '16px 20px', borderBottom: `1px solid ${C.forestBorder}`,
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          }}>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: C.textPrimary }}>💉 Vaccinations</div>
              <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>Next 7 days</div>
            </div>
            {d?.alertCount > 0 && (
              <span style={{
                background: C.redFaint, color: '#E88080',
          ...
          <div style={{ padding: '4px 20px', flex: 1, overflowY: 'auto' }}>
            {loading ? (
              ...
              d.upcomingVaccines.map((v, i) => <VaccineRow key={i} v={v} />)
            ) : (
      ...
      {batchChart?.length > 0 && (
        <div style={{
          ...
          <div style={{ padding: '16px 22px', borderBottom: `1px solid ${C.forestBorder}` }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.textPrimary }}>Batch Profit Comparison</div>
            <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>All time — ranked by profit</div>
          </div>
          <div style={{ padding: '16px 8px 8px' }}>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={batchChart} layout="vertical">
                ...
                <YAxis type="category" dataKey="batchName" tick={{ fontSize: 11, fill: C.textMuted }} axisLine={false} tickLine={false} width={120} />
                <Tooltip content={<ChartTooltip />} />
                <Bar dataKey="profit" name="Profit" radius={[0,4,4,0]} maxBarSize={20}
                  fill={C.greenGlow}
                  label={{ position: 'right', fontSize: 10, fill: C.textMuted, formatter: v => fmt(v) }}
                />
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.textPrimary }}>Active Batches</div>
          ...
          <Link href="/dashboard/batches" style={{
            ...
            border: `1px solid ${C.forestBorder}`,
            borderRadius: 7, background: C.forestSurface2,
          }}>View All</Link>
        </div>

        {loading ? (
          <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[1,2,3].map(i => <Skeleton key={i} h={52} radius={8} />)}
          </div>
          ...
          <div style={{ padding: '48px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 600, color: C.textPrimary, marginBottom: 6 }}>No active batches yet</p>
            <p style={{ fontSize: 13, color: C.textMuted, marginBottom: 20 }}>Create your first batch to start tracking your flock.</p>
            <Link href="/dashboard/batches/new" style={{
              display: 'inline-flex', alignItems: 'center', gap: 6,
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, fontWeight: 700, color: C.textPrimary, marginBottom: 16 }}>Quick Actions</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
          {[
            ...
            <Link key={i} href={action.href} style={{
              ...
              gap: 8, padding: '16px 12px', borderRadius: 10, textDecoration: 'none',
            ...
            >
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: C.textSecondary, textAlign: 'center' }}>{action.label}</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      {d?.finance?.allTime && (
        <div style={{
          ...
          {[
            { label: 'All Time Revenue', value: fmt(d.finance.allTime.revenue),  color: C.greenGlow },
            { label: 'All Time Expenses', value: fmt(d.finance.allTime.expenses), color: '#E88080'  },
            { label: 'All Time Profit',  value: fmt(d.finance.allTime.profit),
            ...
            <div key={i} style={{
              ...
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: C.textMuted, marginBottom: 8 }}>{item.label}</div>
